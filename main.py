...
def saikaiscan():
    ...
    print("Insira a url da novel:")
    entrada = 'https://saikaiscan.com.br/series/tales-of-demons-and-gods-tdg?tab=capitulos'
    print("Insira o nome da novel:")
    #nome_novel = input()
    options = Options()
    #options.headless = True
    ...
    #driver.find_element(By.CLASS_NAME, "btn-reverse-order").click()
    pageSource = driver.page_source
    my_first_epub = pypub.Epub('teste')
    volumesAction = driver.find_elements(By.CLASS_NAME, "active")
    volumes = []
    for volumeAction in volumesAction:
        volumeAction.click()
        volumes.append(driver.find_element(By.CLASS_NAME, "__chapters").get_attribute('innerHTML'))
